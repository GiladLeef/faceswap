...
from faceswap import metadata, wording
from faceswap.uis.typing import Component, ComponentName
...
def load_ui_layout_module(ui_layout : str) -> Any:
	try:
		ui_layout_module = importlib.import_module('faceswap.uis.layouts.' + ui_layout)
...
def launch() -> None:
	with gradio.Blocks(theme = get_theme(), title = metadata.get('name') + ' ' + metadata.get('version'), css="footer {visibility: hidden}") as ui:
		for ui_layout in faceswap.globals.ui_layouts:
			ui_layout_module = load_ui_layout_module(ui_layout)
