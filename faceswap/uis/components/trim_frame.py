...
from faceswap.vision import count_video_frame_total
...
def render() -> None:
	...
	if is_video(faceswap.globals.target_path):
		...
		trim_frame_start_slider_args['value'] = faceswap.globals.trim_frame_start or 0
		trim_frame_start_slider_args['maximum'] = video_frame_total
		trim_frame_start_slider_args['visible'] = True
		trim_frame_end_slider_args['value'] = faceswap.globals.trim_frame_end or video_frame_total
...
def update_trim_frame_start(trim_frame_start : int) -> Update:
	faceswap.globals.trim_frame_start = trim_frame_start if trim_frame_start > 0 else None
	return gradio.update(value = trim_frame_start)


def update_trim_frame_end(trim_frame_end : int) -> Update:
	video_frame_total = count_video_frame_total(faceswap.globals.target_path)
	faceswap.globals.trim_frame_end = trim_frame_end if trim_frame_end < video_frame_total else None
